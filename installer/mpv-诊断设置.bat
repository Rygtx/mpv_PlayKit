...
echo ==   dump 最多保留 5 份，超出自动滚动删除旧文件
echo ==========================================================
...
reg add "%WERKEY%\mpv.exe"           /v DumpCount /t REG_DWORD /d 5 /f >nul
reg add "%WERKEY%\umpv.exe"          /v DumpType  /t REG_DWORD /d 2 /f >nul
reg add "%WERKEY%\umpv.exe"          /v DumpCount /t REG_DWORD /d 5 /f >nul
reg add "%WERKEY%\dlssnr_panel.exe"  /v DumpType  /t REG_DWORD /d 2 /f >nul
reg add "%WERKEY%\dlssnr_panel.exe"  /v DumpCount /t REG_DWORD /d 5 /f >nul
