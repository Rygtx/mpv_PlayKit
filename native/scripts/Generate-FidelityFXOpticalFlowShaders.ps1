# FidelityFX Optical Flow shader blob generator.
# Ported verbatim from Magpie scripts/Generate-FidelityFXOpticalFlowShaders.ps1
# (the ffx_opticalflow_shaderblobs.cpp in vendor/fidelityfx expects exactly
# these header names, so the pass list and the generated C layout must not
# drift). Outputs 7 passes x HDR0/1 = 28 permutation headers; a fingerprint
# stamp makes repeat runs a no-op.
#
# Usage: powershell -File Generate-FidelityFXOpticalFlowShaders.ps1
#            -SdkDirectory <vendor/fidelityfx> -OutputDirectory <gen dir>
#            -DxcPath <Windows SDK dxc.exe>
param(
...
foreach ($directory in @('api\internal\gpu',
    'framegeneration\fsr3\include\gpu',
    'framegeneration\fsr3\internal\shaders')) {
...
foreach ($pass in $passes) {
    ...
    for ($hdr = 0; $hdr -le 1; ++$hdr) {
        ...
        $shader = Join-Path $sdk "framegeneration\fsr3\internal\shaders\$pass.hlsl"
        $arguments = @(
            '-E', 'CS', '-T', 'cs_6_2', '-O3',
            '-D', 'FFX_GPU=1',
            ...
            '-I', (Join-Path $sdk 'api\internal\gpu'),
            '-I', (Join-Path $sdk 'framegeneration\fsr3\include\gpu'),
