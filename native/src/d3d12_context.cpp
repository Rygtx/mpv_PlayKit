...
bool D3D12Context::CreateFrameResources(int width, int height, int depth, bool fg,
    ...
    // temporalOut 由下方 CreateSlotResources 以占位视图重建(temporalOut
    // 纹理本身由 RebuildTemporal 创建)。调用方(RecreateFeature/Rebind)
    // 在本函数成功后按当前 antiFlicker 重新 RebuildTemporal。
    for (auto &t : _tempHist) t.Reset();
    for (auto &t : _tempGuide) t.Reset();
    for (auto &t : _tempLow) t.Reset();
    _temporalMode = 0;
    _temporalSlots = false;
...
bool D3D12Context::CreateSlotResources(FrameSlot &slot, int depth, char *err, size_t errLen) noexcept {
    ...
    if (_fgSlots && !CreateFgSlotResources(slot, err, errLen)) return false;
    {
        ...
        // 49 = 0..34 原有 + 32/33(FG 插值输出的 SRV/UAV;堆空间常备,
        // 非 FG 槽写占位视图)+ 34(共享差异调试纹理 UAV)
        // + 35..48(抗闪烁时域:历史/引导 ping-pong SRV+UAV、temporalOut
        // SRV/UAV、mode4 半分辨率 low SRV+UAV;temporal 关闭时全部为
        ...
        // 35..48:抗闪烁时域占位视图(带 UAV flag 的 outputColor / 普通的
        // inputColor,绝不 NULL —— NULL 描述符异步 TDR,#41-②)。真实的
        // 历史/引导/temporalOut/low 视图由 RebuildTemporal 在 PoolHold 内
        // 覆盖写入;此处槽纹理尚未创建,context 纹理也未建(尺寸可能随
        // CreateFrameResources 变化),占位是唯一安全初值。
        for (UINT i = 35; i <= 38; ++i) {
            _device->CreateShaderResourceView(slot.inputColor.Get(), nullptr, slotHandle(i));
        }
        for (UINT i = 39; i <= 42; ++i) {
            _device->CreateUnorderedAccessView(slot.outputColor.Get(), nullptr, nullptr, slotHandle(i));
        }
        _device->CreateUnorderedAccessView(slot.outputColor.Get(), nullptr, nullptr, slotHandle(43));
        _device->CreateShaderResourceView(slot.outputColor.Get(), nullptr, slotHandle(44));
        for (UINT i = 45; i <= 46; ++i) {
            _device->CreateShaderResourceView(slot.inputColor.Get(), nullptr, slotHandle(i));
        }
        for (UINT i = 47; i <= 48; ++i) {
            _device->CreateUnorderedAccessView(slot.outputColor.Get(), nullptr, nullptr, slotHandle(i));
        }
    }
    return true;
...
// 抗闪烁时域稳定器主 pass(上游 DLSSNRTemporalShader.h 逐行移植,v0.6.8
// 093efe21/55d4cc38;SDR 视频管线砍掉 Hdr 域映射 —— 插件链全程 BGRA8)。
...
bool D3D12Context::CreateComputeObjects(char *err, size_t errLen) noexcept {
    ...
    // 无条件常驻(PSO 生命周期跟着"使用条件"而不是"首次搭车路径",#43-①)。
    {
        ...
        const struct { const char *hlsl; ID3D12PipelineState **pso; } temporalCsos[]{
            { TEMPORAL_MAIN_HLSL, _psoTemporalMain.GetAddressOf() },
            { TEMPORAL_REDUCE_HLSL, _psoTemporalReduce.GetAddressOf() },
        };
        for (const auto &cso : temporalCsos) {
            ComPtr<ID3DBlob> code, csErr;
            if (FAILED(D3DCompile(cso.hlsl, strlen(cso.hlsl), nullptr, nullptr, nullptr,
                                  "main", "cs_5_0", 0, 0, code.GetAddressOf(), csErr.GetAddressOf()))) {
...
bool D3D12Context::RebuildTemporal(int mode, char *err, size_t errLen) noexcept {
    ...
    _temporalMode = 0;
    if (mode == 0) {
        // 视图回占位(inputColor/outputColor —— placeholder 惯例,绝 NULL 描述符)。
        const UINT inc = _device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
        for (int i = 0; i < kSlotCount; ++i) {
    ...
    // 每槽 temporalOut(BGRA8 UAV,COMMON 起步;纹理先于视图 —— NULL 描述符
    // 异步 TDR 防线,#50 同款顺序铁律)。
    for (int i = 0; i < kSlotCount; ++i) {
    ...
    for (int i = 0; i < kSlotCount; ++i) {
        const D3D12_CPU_DESCRIPTOR_HANDLE base = _slots[i].srvUavHeap->GetCPUDescriptorHandleForHeapStart();
        auto h = [&](UINT d) { return D3D12_CPU_DESCRIPTOR_HANDLE{ base.ptr + static_cast<SIZE_T>(d * inc) }; };
        _device->CreateShaderResourceView(_tempHist[0].Get(), nullptr, h(35));
        _device->CreateShaderResourceView(_tempHist[1].Get(), nullptr, h(36));
        _device->CreateShaderResourceView(_tempGuide[0].Get(), nullptr, h(37));
        _device->CreateShaderResourceView(_tempGuide[1].Get(), nullptr, h(38));
        _device->CreateUnorderedAccessView(_tempHist[0].Get(), nullptr, nullptr, h(39));
        _device->CreateUnorderedAccessView(_tempHist[1].Get(), nullptr, nullptr, h(40));
        _device->CreateUnorderedAccessView(_tempGuide[0].Get(), nullptr, nullptr, h(41));
        _device->CreateUnorderedAccessView(_tempGuide[1].Get(), nullptr, nullptr, h(42));
        _device->CreateUnorderedAccessView(_slots[i].temporalOut.Get(), nullptr, nullptr, h(43));
        _device->CreateShaderResourceView(_slots[i].temporalOut.Get(), nullptr, h(44));
        ...
        if (mode == 4) {
            _device->CreateShaderResourceView(_tempLow[0].Get(), nullptr, h(45));
            _device->CreateShaderResourceView(_tempLow[1].Get(), nullptr, h(46));
            _device->CreateUnorderedAccessView(_tempLow[0].Get(), nullptr, nullptr, h(47));
            _device->CreateUnorderedAccessView(_tempLow[1].Get(), nullptr, nullptr, h(48));
        } else {
            _device->CreateShaderResourceView(_slots[i].inputColor.Get(), nullptr, h(45));
            _device->CreateShaderResourceView(_slots[i].inputColor.Get(), nullptr, h(46));
            _device->CreateUnorderedAccessView(_slots[i].outputColor.Get(), nullptr, nullptr, h(47));
            _device->CreateUnorderedAccessView(_slots[i].outputColor.Get(), nullptr, nullptr, h(48));
        }
    }
    _temporalMode = mode;
    char msg[128];
...
void D3D12Context::RecordTemporal(FrameSlot &slot, int mode, int next, bool useMotion,
    ...
    // 上游 DLSSNRTemporal::Draw 的 D3D12 移植:mode4 先 reduce(半分辨率
    // 残差+引导)后主 pass。t0/t1 都绑 inputColor(单 pass 链 base=输入,
    // 上游 count==1 时 input/base 同纹理);t2 = outputColor(NR 链原始
    // 输出)。motion 已由 guidance 段转入 NSR(nvof CL 的 postExecute 或
    // 槽 CL 降采样路径);useMotion=false 时 t5 照绑(占位),shader 侧
    // UseMotion=0 全路径守卫绝不读取。
    ID3D12GraphicsCommandList *cl = slot.commandList.Get();
    ...
    if (lowNeeded) {
        ...
        cl->SetComputeRootDescriptorTable(9, gpu(47));
        cl->SetComputeRootDescriptorTable(10, gpu(48));
    ...
    cl->SetComputeRootDescriptorTable(4, gpu(35 + previous));         // t3 History
    cl->SetComputeRootDescriptorTable(5, gpu(37 + previous));         // t4 PreviousGuide
    cl->SetComputeRootDescriptorTable(6, gpu(motionSrvIndex));        // t5 Motion
    cl->SetComputeRootDescriptorTable(7, lowNeeded ? gpu(45) : gpu(35 + previous)); // t6
    cl->SetComputeRootDescriptorTable(8, lowNeeded ? gpu(46) : gpu(37 + previous)); // t7
    cl->SetComputeRootDescriptorTable(9, gpu(43));                    // u0 Output
    cl->SetComputeRootDescriptorTable(10, gpu(39 + next));            // u1 NextHistory
    cl->SetComputeRootDescriptorTable(11, gpu(41 + next));            // u2 NextGuide
    cl->Dispatch((sizeWH[0] + 7) / 8, (sizeWH[1] + 7) / 8, 1);

    // 归位:input/output/历史/low 回 COMMON(帧末全资源 COMMON 不变量);
    // temporalOut 留 UAV(RecordYuvOutput stateBefore=UAV 消费后归位);
    // motion 留 NSR(recordGuidancePark 按既有契约归位)。
