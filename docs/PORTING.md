...
- 通信:面板与插件经两块 512 字节命名共享内存双向通信(`native/src/panel_ipc.h`:参数通道面板写 → 插件 `bridge.cpp` 40ms 轮询应用(seq 门控,下一帧生效);stats 通道插件每帧写 → 面板 0.1s 读,即 Profiler 数据源);「保存设置」→ `dlssnr_ui.ini` → 下次加载滤镜作为默认值(优先于 .vpy;删除 ini 恢复 vpy 控制)。test_bridge 9/9 字段、test_lifecycle 生命周期验证通过。
...
| 8 | DLSS FG RTX 40/50 官方 NGX 分支(Magpie 式,非 proxy):`Init_with_ProjectID`(自定义 ProjectID + PathList)→ 部署 DLSS SDK 官方签名 `nvngx_dlssg.dll`(SM89/SM120 原生 cubin,无 PTX JIT)→ `NGX_D3D12_CREATE_DLSSG`/`EVALUATE_DLSSG` + NVOF 自产运动矢量;官方 DLSSG 无需 IAT hook(hook 仅 DLSSNR 签名 snippet 的 GetModuleFileNameW 路径校验需要)。现有 proxy 分支(dlssg_for_sm86 version.dll)退化为 RTX 30/20 专用——官方 DLSSG 在 Ampere/Turing 拒载(FrameGeneration_Available=0),proxy 是该区间唯一路径 | NgxD3D12Core.cpp:18-34、DLSSFrameGenerator.cpp(NGX_D3D12_CREATE_DLSSG/EVALUATE + FrameGuidance 供 MV)、Magpie.vcxproj:739(SDK rel nvngx_dlssg.dll 部署) | 高:40/50 走官方签名链,免自签证书与杀软误报面;dlssfg_context 能力预检键面(?DLSSG.Available/NeedsUpdatedDriver/MultiFrameCountMax)与官方 NGX 同键,FG MVecs 契约已要求按源尺寸建 NVOF 会话,接入面现成 | 中 | **功能完成**(2026-09-11):DlssfgContext 双后端化(Proxy/OfficialNgx),冷初始化官方优先、按 capability 自动分流——`nvngx_dlssg.dll` 与模型同目录部署且驱动报告 FG 能力时走官方链(参数块 = GetCapabilityParameters,Magpie 同款;函数指针指向静态 SDK,与 proxy 导出同签名);不可用回落 proxy。eval 键面补全官方契约(Magpie optionalParams:五矩阵恒等/相机单位基座/jitter 0/可选资源 null/子矩形;proxy 忽略未知键无害)。fetch-deps.ps1 自动落地官方签名 runtime 到 vendor\ngx\(~7.4MB,尺寸门槛防 HTML/LFS 指针污染),package.ps1 选装。**3080 实测**(SM86 回归):官方分支真实触发 → capability 拒载(FeatureInitResult 0xBAD0000B;NVGX log 双证 `snippet arch 190 > 170`)→ 回落 proxy,x3 输出正常;NR 项目映射零回退(`Found cms id 876232c`,坑 #3 共存验证过);两后端皆缺 → 1:1(test_fg_fallback)。**RTX 40/50 实卡待验证** |

### 建议顺序

1 → 4 → 5 → 6;7 不计划;8 已完成(#6 NVOF 会话链就位,双后端按能力自动分流:40/50 官方 NGX,30/20 proxy)。(1 与 4/5 无依赖,可并行评估;2 已作废并入 6)
