...
| `Motion_Vector_Quality` | 0–5 | 0 | NVOF optical-flow guidance level (0 = zero guidance; 1–5 use hardware optical flow to reduce motion-scene temporal artifacts; higher is more accurate but slower). Consumed only when the backend is NVOF |
| `Ffx_Quality` | 0–2 | 0 | FFX optical-flow guidance level (0 = zero guidance; 1 = performance / half-resolution OF; 2 = quality / full resolution). Consumed only when the backend is FFX (default). The two fields are independent; the panel's single "optical-flow quality" dropdown reads/writes the field matching the current backend |
...
| `Of_Backend` | 0–1 | 0 | Optical-flow backend (0 = FFX / AMD FidelityFX, cross-vendor, default; 1 = NVOF / NVIDIA-exclusive engine). Switching affects the next optical-flow session creation (next frame / seek) |
...
|---|---|---|
| Optical-flow guidance levels 1–5 | FFX: D3D12 SM6.2 + WaveOps + R16G16_SINT UAV (cross-vendor); NVOF: RTX Turing+ | Session creation failure auto-falls back to zero guidance; nothing else is blocked |
...
- **DLSS frame generation**: a single official NGX chain (Magpie-style: the NVIDIA-signed `nvngx_dlssg.dll` through the shared NGX core + NVOF-generated motion vectors); on RTX 30/20 the [dlssg_for_sm86](https://github.com/sdli1995/dlssg_for_sm86) **0.3.x hook proxy** delivers DLSS-G (the version.dll installs its hooks at LoadLibrary: intercepts the `nvngx_dlssg.dll` load and swaps in its embedded runtime, kernels auto-picked per physical architecture, tier 1 bit-identical to official); on RTX 40 the official runtime is used — NVIDIA caps 40-series at 2x via an in-library count gate (software policy; the MFG model itself works, the same DLL ships native multi-frame for 50-series), and the plugin automatically unlocks it in-process for 3–6x (technique from [RTX40MFG-Unlock](https://github.com/dashdogy/RTX40MFG-Unlock), MIT; process memory only, falls back to 2x on a miss). The panel "FG route" has two gears: Auto (GPU-family split: 30/20 preload the proxy, 40+ go official + unlock) / Pure official (no preload, straight to the official runtime, same unlock) — process-level, mpv restart required after switching

| GPU | Architecture | DLSSNR denoise | DLSS frame gen |
|---|---|---|---|
| RTX 50 | Blackwell (SM120) | 🟡 Theoretical (official range, untested) | 🟡 Theoretical (official NGX branch, native MFG, untested) |
| RTX 40 | Ada (SM89) | 🟡 Theoretical (official range, untested) | 🟡 Theoretical (official NGX branch + in-process mfg gate unlock for 3–6x, technique verified upstream, untested with this plugin; falls back to 2x on a miss) |
...
> - Hardware optical-flow guidance defaults to FFX (AMD FidelityFX, cross-vendor — AMD/Intel cards work too; requires D3D12 SM6.2 + WaveOps); NVOF (NVIDIA Turing+) is the opt-in second backend; unsupported hardware auto-falls back to zero guidance without blocking the feature (the failure reason is carried by `of_detail` on the diagnostics page)
> - The official chain requires `nvngx_dlssg.dll` (NVIDIA-signed, included in the release package) next to the model DLL; Auto mode splits by GPU family (NVAPI architecture probe: on 30/20 it preloads the 0.3.x proxy before init so its hooks land before the capability query — one clean pass, no retry; on 40 and newer it skips the proxy and drives official + gate unlock), while the "Pure official" gear skips the preload (on 30/20 the architecture gate rejects with 0xBAD0000B, FG off, `fg_detail` on the diagnostics page carries the reason)
> - RTX 40-series 6x note: the official runtime caps 40-series at 2x (two layers: the in-library count gate + the driver per-game override; the latter only applies to Streamline games, which mpv's direct-drive path never touches). The unlock fires **on Ada (SM89) only** (NVAPI architecture probe; 50-series ships native MFG and the official runtime is left byte-for-byte untouched; non-Ada or probe failure always skips). When triggered, the plugin applies a 2-byte in-process gate unlock to `nvngx_dlssg.dll` (unique match + target-instruction double anchor; any failure leaves the value untouched), and both the unlock result and the cap go into the timing log (`dlssfg mfg gate unlocked` / `pattern not found` / `unlock skipped (not Ada)`)
