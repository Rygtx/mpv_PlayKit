...
| RTX 40-series 3–6x frame generation | the byte signature of the count gate inside the official `nvngx_dlssg.dll` matches (in-process unlock by the plugin, same technique as RTX40MFG-Unlock; process memory only, nothing written to disk) | On a signature miss (driver update rewrites the runtime) it falls back to 2x automatically; the timing log carries the reason |
...
> - RTX 40-series 6x note: the official runtime caps 40-series at 2x (two layers: the in-library count gate + the driver per-game override; the latter only applies to Streamline games, which mpv's direct-drive path never touches). When the capability query reports less than the cap, the plugin applies a 2-byte in-process gate unlock to `nvngx_dlssg.dll` (unique match + target-instruction double anchor; any failure leaves the value untouched), and both the unlock result and the cap go into the timing log (`dlssfg mfg gate unlocked` / `pattern not found`)
